...
def generate_max_contig(sg, seqs, out_fn=None):
    ...
    while len(G) > 0:
        ...
        for i in range( 0, len( path ) - 1 ):
            ...
            if (w_n, v_n) in g_edges:
                cmp_edges.add( (w_n, v_n) )
...
def generate_meta_unitig(sg, uni_edges, seqs, out_fn, connected_nodes = None):
    ...
    for v,w in uni_edges.keys():
        if max( [len(s[0]) for s in  uni_edges[ (v,w) ] ] ) < 3 and \
            len(out_edges[w]) ==0 or len(in_edges[v]) ==0:
                continue
        uni_graph.add_edge(v, w, n_weight = max_weight - max( [len(s[1]) for s in  uni_edges[ (v,w) ] ] ) )
    ...
    while len(G) > 0:
        ...
        print count, len(candidates), len(G.nodes()), candidate, len(path), len(new_path)
        if len(new_path) > 1:
            path = new_path
            subseqs = []
            for i in range(len(path) - 1):
